...
type MessageSentListener = (payload: MessageSentPayload) => void;
type MessageDeliveryListener = (payload: MessageDeliveryPayload) => void;
type MessageReadListener = (payload: MessageReadPayload) => void;
type ConversationReadListener = (payload: ConversationReadPayload) => void;
type ConversationReadAckListener = (payload: ConversationReadAckPayload) => void;
type PresenceUpdateListener = (payload: PresenceUpdate) => void;
type PresenceResponseListener = (payload: PresenceResponse) => void;
type OnlineUsersResponseListener = (payload: OnlineUsersResponse) => void;
type PongListener = (payload: PongPayload) => void;
...
class MessageSocketClient {
  ...
  private typingStartListeners = new Set<TypingListener>();
  private typingStopListeners = new Set<TypingListener>();
  private messageSentListeners = new Set<MessageSentListener>();
  private messageDeliveryListeners = new Set<MessageDeliveryListener>();
  private messageReadListeners = new Set<MessageReadListener>();
  private conversationReadListeners = new Set<ConversationReadListener>();
  private conversationReadAckListeners = new Set<ConversationReadAckListener>();
  private presenceUpdateListeners = new Set<PresenceUpdateListener>();
  private presenceResponseListeners = new Set<PresenceResponseListener>();
  private onlineUsersResponseListeners = new Set<OnlineUsersResponseListener>();
  private pongListeners = new Set<PongListener>();
  ...
  onTypingStart(listener: TypingListener): () => void {
    ...
    return () => this.typingStartListeners.delete(listener);
  }

  onTypingStop(listener: TypingListener): () => void {
    this.typingStopListeners.add(listener);
    return () => this.typingStopListeners.delete(listener);
  }

  onMessageSent(listener: MessageSentListener): () => void {
    this.messageSentListeners.add(listener);
    return () => this.messageSentListeners.delete(listener);
  }

  onMessageDelivery(listener: MessageDeliveryListener): () => void {
    this.messageDeliveryListeners.add(listener);
    return () => this.messageDeliveryListeners.delete(listener);
  }

  onMessageRead(listener: MessageReadListener): () => void {
    this.messageReadListeners.add(listener);
    return () => this.messageReadListeners.delete(listener);
  }

  onConversationRead(listener: ConversationReadListener): () => void {
    this.conversationReadListeners.add(listener);
    return () => this.conversationReadListeners.delete(listener);
  }

  onConversationReadAck(listener: ConversationReadAckListener): () => void {
    this.conversationReadAckListeners.add(listener);
    return () => this.conversationReadAckListeners.delete(listener);
  }

  onPresenceUpdate(listener: PresenceUpdateListener): () => void {
    this.presenceUpdateListeners.add(listener);
    return () => this.presenceUpdateListeners.delete(listener);
  }

  onPresenceResponse(listener: PresenceResponseListener): () => void {
    this.presenceResponseListeners.add(listener);
    return () => this.presenceResponseListeners.delete(listener);
  }

  onOnlineUsersResponse(listener: OnlineUsersResponseListener): () => void {
    this.onlineUsersResponseListeners.add(listener);
    return () => this.onlineUsersResponseListeners.delete(listener);
  }

  onPong(listener: PongListener): () => void {
