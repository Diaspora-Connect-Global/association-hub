// TypeScript types for the Post & Feed service (association-hub adapted)
...
export type FeedType = "PERSONAL" | "COMMUNITY" | "ASSOCIATION" | "TRENDING" | "DISCOVER";
...
export interface AssociationPostAttachmentInput extends PostAttachmentInput {
  size: number;
  duration?: number;
}
...
export interface CreateAssociationPostInput {
  associationId: string;
  text: string;
  visibility?: PostVisibility;
  attachments?: AssociationPostAttachmentInput[];
  mentionedUserIds?: string[];
  mentions?: MentionInput[];
}
...
export interface AssociationFeedResponse extends PostListResponse {
  hasMore: boolean;
}
