...
interface JoinMembershipSectionProps {
  ...
  /** Display name shown in UI (e.g. "Ghana Nurses Association") */
  entityName: string;
  /** Payment amount in smallest currency unit (e.g. cents). Pass 0 if free. */
  amount?: number;
...
export function JoinMembershipSection({
  ...
  const loadMembership = useCallback(async () => {
    try {
      ...
      // No existing membership is fine
      setMembership(null);
    } finally {
  ...
  const startPolling = useCallback(() => {
    ...
    pollTimeoutRef.current = setTimeout(() => {
      ...
      toast({
        ...
        description: "Your payment was successful. Refresh the page in a moment to check your membership status.",
        variant: "destructive",
      });
      void loadMembership();
    }, POLL_TIMEOUT_MS);
  }, [entityId, entityType, entityName, loadMembership, stopPolling]);
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      if (result.requiresPayment && result.clientSecret) {
        ...
        // Free join or request submitted
        setMembership({
          id: result.id,
  ...
  const isPending = isPendingReview || isPendingPayment;
  const hasSubscription = isActive && !!membership?.subscriptionId;
    ...
    <>
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {isPendingPayment && (
            <div className="space-y-2">
              ...
              <Button size="sm" onClick={() => { void handleJoin(); }} disabled={isJoining}>
                {isJoining ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Submitting…</> : "Complete Payment"}
              </Button>
            </div>
          )}
          ...
          {!membership && !isPolling && (
            <Button onClick={() => { void handleJoin(); }} disabled={isJoining} className="w-full sm:w-auto">
              {isJoining ? (
                <>
