/**
 * Canonical catalog of member-facing "services" that an association can enable
 * for its members. This is the single source of truth used by:
 *  - Settings → Services panel (per-association module toggles)
 *
 * The `key` values are the contract with the backend
 * (`enabledServices: [String!]`). Labels/descriptions here are plain English
 * strings used directly by the UI (this repo has no per-service i18n keys).
 */
import {
  Newspaper,
  Calendar,
...
export const COMMUNITY_SERVICES: readonly CommunityServiceDef[] = [
  ...
  },
  {
    ...
    description: "Requests members submit for association-provided services.",
    icon: ClipboardList,
  },
  {
    key: "resources",
